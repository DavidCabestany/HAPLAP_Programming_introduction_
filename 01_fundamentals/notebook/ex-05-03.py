def londons_weather(temp: int, iPath: str, oPath: str):
    ...
    txt1 = "Total entries: {}\nAverage temperature: {}\nTotal precipitation: {}\nAverage wind-speed: {}".format(
        t_entry, format(temp_av, ".2f"), format(s_rain, ".2f"), format(wind_av, ".2f")
    )

    oFile = iFile[
        ["Year", "Month", "Day", "Temperature", "Precipitation", "Wind-speed"]
    ]
    df_tail = iFile[["Temperature", "Precipitation", "Wind-speed"]]

    df_head = oFile.assign(
        Date=oFile.Year.astype(str)
        + "-"
        + oFile.Month.astype(str)
        + "- "
        + oFile.Day.astype(str)
    )
